    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Videos @if($isOpenOrder) - Ordering @endif</h3>
            ...
            @if($isOpenOrder)
            @include('partners.course.lesson.video.component-order')
            @else
            <div class="card-body">
                <table class="table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right py-0 align-middle">
                                <div class="btn-group btn-group-sm">
                                    <button data-toggle="modal" wire:click="setVideo({{$video->id}})" data-target="#modal-update" class="btn btn-warning"><i class="fas fa-edit"></i></button>
                                    <button data-id="{{$video->id}}" class="btn btn-danger btn-delete btn-process"><i class="fas fa-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
                ...
                <form wire:submit.prevent="upload" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-error="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                    <div class="modal-body">
                        ...
                        <div class="w-100 mt-2">
                            ...
                            <input type="file" wire:model="file" accept=".mp4,.mkv" class="form-control" id="upload{{$iteration}}" required>
                        </div>
                        @error('file')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
            ...
            <div class="modal-content">
                ...
                <form wire:submit.prevent="update" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true" x-on:livewire-upload-finish="isUploading = false" x-on:livewire-upload-error="isUploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                    <div class="modal-body">
                        <div class="w-100">
                            <label for="">Title</label>
                            <input type="text" wire:model.defer="video.title" class="form-control" required>
                        </div>
                        @error('title')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                        <div class="w-100 mt-2">
                            ...
                            <input type="file" wire:model="update_file" accept=".mp4,.mkv" class="form-control" id="upload_update_{{$iteration}}">
                        </div>
                        @error('update_file')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
